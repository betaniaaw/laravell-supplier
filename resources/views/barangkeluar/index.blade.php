  ...
  <table class="w-full text-left table-auto min-w-max">
    ...
    <th class="p-4 border-b border-slate-200 bg-slate-50">
      ...
      <tr class="hover:bg-slate-50 border-b border-slate-200">
          ...
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $barangkeluar->nama_barang }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $barangkeluar->tgl_keluar }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $barangkeluar->jml_keluar }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $barangkeluar->lokasi }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $barangkeluar->penerima }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500"><a href="{{ route('barangkeluar.edit', $barangkeluar->id) }}" class="font-medium text-blue-600 hover:text-blue-800">Edit</a></p>
          </td>
          <td class="p-4 py-5">
           <p class="text-sm text-slate-500">
               <a href="{{ route('barangkeluar.destroy', $barangkeluar->id) }}"
                   class="font-medium text-blue-600 hover:text-blue-800"
                   onclick="event.preventDefault(); if(confirm('Are you sure you want to delete this barang?')) { document.getElementById('delete-form-{{ $barangkeluar->id }}').submit(); }">
                    Hapus
                </a>
                <form id="delete-form-{{ $barangkeluar->id }}" action="{{ route('barangkeluar.destroy', $barangkeluar->id) }}" method="POST" style="display: none;">
                   @csrf
                   @method('DELETE')
               </form>
           </p>
