...
<div class="max-w-[720px] mx-auto py-32 sm:py-8 lg:py-16">
  ...
  <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
      <div>
          <h3 class="text-lg font-semibold text-slate-800">Data Stok</h3>
  ...
  <table class="w-full text-left table-auto min-w-max">
      ...
      <tr class="hover:bg-slate-50 border-b border-slate-200">
          ...
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $stok->id_barang }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $stok->nama_barang}}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $stok->spesifikasi }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $stok->lokasi }}</p>
          </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500">{{ $stok->kondisi }}</p>
          </td>
          <td class="p-4 py-5">
            <p class="text-sm text-slate-500">{{ $stok->jumlah_barang }}</p>
            </td>
            <td class="p-4 py-5">
                <p class="text-sm text-slate-500">{{ $stok->sumber_dana }}</p>
                </td>
          <td class="p-4 py-5">
          <p class="text-sm text-slate-500"><a href="{{ route('stok.edit', $stok->id) }}" class="font-medium text-blue-600 hover:text-blue-800">Edit</a></p>
          </td>
          <td class="p-4 py-5">
           <p class="text-sm text-slate-500">
               <a href="{{ route('stok.destroy', $stok->id) }}"
                   class="font-medium text-blue-600 hover:text-blue-800"
                   onclick="event.preventDefault(); if(confirm('Are you sure you want to delete this barang?')) { document.getElementById('delete-form-{{ $stok->id }}').submit(); }">
                    Hapus
                </a>
